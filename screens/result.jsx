...
export default function Result({route}){
    ...
    const {list , userId} = route.params;
    const data = [
        {
            "solution_id" : 1,
            "title" : "차마시기",
    ...
    return(
        <View style={styles.container}>
            ...
            {data.map((item, index) => {
                return list.includes(item.solution_id) ? (
                <TouchableOpacity
                    key={index}
                    style={styles.section}
                    onPress={() => {
                    navigation.navigate('Detail', { id: [item.solution_id] , userId : userId});
                    }}
                >
                    <Text style={styles.title}>{item.title}</Text>
                    <Image source={item.src} style={[{width:285}, {borderRadius:10}, {marginTop:10}]}/>
                </TouchableOpacity>
                ) : null
            })}
            ...
            <Footer people={false} home={true} storage={false} id = {userId} />
        </View>
    )
}
